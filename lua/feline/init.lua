...
local function setup_common(config, is_winbar)
    ...
    if is_winbar then
        ...
        module.use_autocmd = config.use_autocmd
    else
        M.statusline_module = {}
        ...
        utils.create_augroup({
            {
                ...
                opts = {
                    callback = function()
                        local content = require('feline').generate_winbar(true)
        ...
        utils.create_augroup({
            {
                ...
                opts = {
                    callback = function()
                        local content = require('feline').generate_winbar(false)
                        local status_ok, _ = pcall(api.nvim_set_option_value, "winbar", content, { scope = "local" })
                        if not status_ok then
                            return
                        end
...
function M.generate_winbar(active)
    local is_active = active
    if not M.winbar_module.use_autocmd then
        is_active = api.nvim_get_current_win() == tonumber(vim.g.actual_curwin)
    end
